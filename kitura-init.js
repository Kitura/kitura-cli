...
/*
Projects created with kitura init start by cloning a template project, and renaming elements of that project to match the user's chosen project name (determined by the name of their current directory).
We perform this renaming for three variants of the project's name: The original name as given, the same name with special characters stripped (as required by SwiftPM), and the stripped name also lowercased (used for example in the name of Docker containers generated by 'kitura build' and 'kitura run').
*/
function renameProject() {
    ...
    //Construct a regex expression to replace multiple occurrences of oldProjName
